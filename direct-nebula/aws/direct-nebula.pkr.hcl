# Follow the steps below to generate direct-nebula ami
# 1. cp .env.example .env
# 2. Update .env with appropriate values
# 3. source .env
# 4. AWS_PROFILE=<AMI Profile Name> packer build direct-nebula.pkr.hcl
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo chown -R ${local.service_user}:${local.service_user} /home/${local.service_user}",
      "sudo chmod +x /home/${local.service_user}/direct-nebula",
