...
function publish() {
  ...
  for plat in darwin_amd64 darwin_arm64 linux_amd64 linux_arm64 windows_amd64; do
    src_file=${PRODUCT}-${VERSION}-${BLD_NUM}_${plat}.zip
    tgt_file=${PRODUCT}-${VERSION}_${plat}.zip
    curl --fail ${LATESTBUILD_URL}/${PRODUCT}/${VERSION}/${BLD_NUM}/${src_file} \
      -o ${tgt_file}
    sha256sum ${tgt_file} >> ${PRODUCT}-${VERSION}_checksums.txt
  done

  files=$(ls *.zip *_checksums.txt)
