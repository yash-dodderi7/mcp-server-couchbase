...
# Simple script for building vault-plugin-database-couchbasecapella
...
function install_deps() {
  ...
  ./cbdep install golang ${GO_VER} -d extra
  export PATH=`pwd`/extra/go${GO_VER}/bin:$PATH
}
    ...
    case "${OSTYPE}" in
      linux)
        for ARCH in amd64 arm64; do
          GOOS=${OSTYPE} GOARCH=${ARCH} go build \
            -o couchbasecapella-database-plugin ./cmd/couchbasecapella-database-plugin/. 
          zip ${PRODUCT}-${VERSION}-${BLD_NUM}_${OSTYPE}_${ARCH}.zip \
            couchbasecapella-database-plugin
        done
        GOARM=6 GOARCH=arm GOOS=${OSTYPE} go build \
          -o couchbasecapella-database-plugin ./cmd/couchbasecapella-database-plugin/. 
        zip ${PRODUCT}-${VERSION}-${BLD_NUM}_${OSTYPE}_${ARCH}.zip \
          couchbasecapella-database-plugin
        ;;
      ...
      windows)
        ARCH=amd64
        GOOS=${OSTYPE} GOARCH=${ARCH} go build \
          -o couchbasecapella-database-plugin.exe ./cmd/couchbasecapella-database-plugin/. 
        zip ${PRODUCT}-${VERSION}-${BLD_NUM}_${OSTYPE}_${ARCH}.zip \
          couchbasecapella-database-plugin.exe
        ;;
      *)
        echo "unknown: $OSTYPE"
        exit 1
